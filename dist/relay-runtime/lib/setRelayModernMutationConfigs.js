...
function rangeAdd(config, operation) {
  ...
  if (!parentID) {
    process.env.NODE_ENV !== 'production' ? warning(false, 'setRelayModernMutationConfigs: For mutation config RANGE_ADD ' + 'to work you must include a parentID') : void 0;
    return;
  }
  var rootField = getRootField(operation);
            ...
            if (serverEdge) {
              ...
              switch (info.rangeBehavior) {
                ...
                case 'prepend':
                  ...
                  break;
                default:
                  process.env.NODE_ENV !== 'production' ? warning(false, 'setRelayModernMutationConfigs: RANGE_ADD range behavior ' + ('\'' + info.rangeBehavior + '\' will not work as expected in RelayModern, ') + "supported range behaviors are 'append', 'prepend', and " + "'ignore'") : void 0;
                  break;
              }
            }
          }
...
function rangeDelete(config, operation) {
  ...
  if (!parentID) {
    process.env.NODE_ENV !== 'production' ? warning(false, 'setRelayModernMutationConfigs: For mutation config RANGE_DELETE ' + 'to work you must include a parentID') : void 0;
    return;
  }
  var rootField = getRootField(operation);
  if (rootField) {
...
function deleteNode(parentID, connectionKeys, pathToConnection, store, deleteIDs) {
  process.env.NODE_ENV !== 'production' ? warning(connectionKeys, 'setRelayModernMutationConfigs: RANGE_DELETE must provide a ' + 'connectionKeys') : void 0;
  var parent = store.get(parentID);
  if (!parent) {
    return;
  }
  if (pathToConnection.length >= 2) {
    ...
    if (connectionKeys && recordProxy) {
      ...
      process.env.NODE_ENV !== 'production' ? warning(false, 'setRelayModernMutationConfigs: RANGE_DELETE ' + 'pathToConnection is incorrect. Unable to find connection with ' + 'parentID: %s and path: %s', parentID, pathToConnection.toString()) : void 0;
    }
  } else {
    process.env.NODE_ENV !== 'production' ? warning(false, 'setRelayModernMutationConfigs: RANGE_DELETE ' + 'pathToConnection must include at least parent and connection') : void 0;
  }
}
