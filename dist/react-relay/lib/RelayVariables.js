  ...
  fragment.argumentDefinitions.forEach(function (definition) {
    ...
    switch (definition.kind) {
      ...
      case 'RootArgument':
        ...
        break;
      default:
        !false ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayVariables: Unexpected node kind `%s` in fragment `%s`.', definition.kind, fragment.node.name) : invariant(false) : void 0;
    }
  });
...
function getFragmentSpreadArguments(fragmentName, variableMapping, parentVariables, rootVariables) {
  return mapObject(variableMapping, function (value, name) {
    ...
    if (value === undefined) {
      process.env.NODE_ENV !== 'production' ? warning(false, 'RelayVariables.getFragmentSpreadArguments(): Variable `%s` is ' + 'undefined in fragment `%s`.', name, fragmentName) : void 0;
      value = null;
    }
    return value;
  });
