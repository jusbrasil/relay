...
function findConnectionMetadata(fragments) {
  ...
  for (var _fragmentName in fragments) {
    ...
    if (connectionMetadata) {
      !(connectionMetadata.length === 1) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'ReactRelayPaginationContainer: Only a single @connection is ' + 'supported, `%s` has %s.', _fragmentName, connectionMetadata.length) : invariant(false) : void 0;
...
function assertRelayContext(relay) {
  !isRelayContext(relay) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'ReactRelayPaginationContainer: Expected `context.relay` to be an object ' + 'conforming to the `RelayContext` interface, got `%s`.', relay) : invariant(false) : void 0;
  return relay;
}
