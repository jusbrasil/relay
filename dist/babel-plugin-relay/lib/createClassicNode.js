      ...
      if (directives.length === 0) {
        ...
        switch (directive.name.value) {
          ...
          case 'relay':
            ...
            !(relayArguments.length === 1 && relayArguments[0].name.value === 'mask') ? process.env.NODE_ENV !== 'production' ? invariant(false, 'BabelPluginRelay: Expected `@relay` directive to only have `mask` argument in ' + 'compat mode, but get %s', relayArguments[0].name.value) : invariant(false) : void 0;
            substitutionName = fragmentName;
...
function createSubstitutionsForFragmentSpreads(t, path, fragments) {
  return Object.keys(fragments).map(function (varName) {
    ...
    if (!fragment.isMasked) {
      !(path.scope.hasBinding(module) || path.scope.hasBinding(propName)) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'BabelPluginRelay: Please make sure module \'' + module + '\' is imported and not renamed or the\n        fragment \'' + fragment.name + '\' is defined and bound to local variable \'' + propName + '\'. ') : invariant(false) : void 0;
