...
var GraphQLCompilerContext = function () {
  ...
  GraphQLCompilerContext.prototype.add = function add(node) {
    !!this._documents.has(node.name) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLCompilerContext: Duplicate document named `%s`. GraphQL ' + 'fragments and roots must have unique names.', node.name) : invariant(false) : void 0;
  ...
  GraphQLCompilerContext.prototype.getRoot = function getRoot(name) {
    var record = this._documents.get(name);
    var node = record && record.get('node');
    !(node && node.kind === 'Root') ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLCompilerContext: Expected `%s` to be a root, got `%s`.', name, node && node.kind) : invariant(false) : void 0;
    return node;
  };
  ...
  GraphQLCompilerContext.prototype._get = function _get(name) {
    var record = this._documents.get(name);
    !record ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLCompilerContext: Unknown document `%s`.', name) : invariant(false) : void 0;
    return record;
  };
