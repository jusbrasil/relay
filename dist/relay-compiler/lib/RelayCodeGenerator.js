...
var prettyStringify = require('./prettyStringify');
...
function generate(node) {
  !(['Root', 'Fragment'].indexOf(node.kind) >= 0) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayCodeGenerator: Unknown AST kind `%s`. Source: %s.', node.kind, getErrorMessage(node)) : invariant(false) : void 0;
...
var RelayCodeGenVisitor = {
  leave: {
    ...
    Argument: function Argument(node, key, parent, ancestors) {
      !(['Variable', 'Literal'].indexOf(node.value.kind) >= 0) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayCodeGenerator: Complex argument values (Lists or ' + 'InputObjects with nested variables) are not supported, argument ' + '`%s` had value `%s`. Source: %s.', node.name, prettyStringify(node.value), getErrorMessage(ancestors[0])) : invariant(false) : void 0;
