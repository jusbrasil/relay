...
function getFragmentScope(definitions, args, parentScope) {
  ...
  definitions.forEach(function (definition) {
    if (definition.kind === 'RootArgumentDefinition') {
      !!argMap.hasOwnProperty(definition.name) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayCompilerScope: Unexpected argument for global variable `%s`. ' + '@arguments may only be provided for variables defined in the ' + "fragment's @argumentDefinitions list.", definition.name) : invariant(false) : void 0;
      ...
      if (arg == null || arg.kind === 'Literal' && arg.value == null) {
        ...
        // value.
        !(definition.defaultValue != null || !(definition.type instanceof GraphQLNonNull)) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayCompilerScope: No value found for required argument ' + '`$%s: %s`.', definition.name, definition.type.toString()) : invariant(false) : void 0;
        fragmentScope[definition.name] = {
          kind: 'Literal',
