...
function visitField(field, state) {
  ...
  field.name === SchemaMetaFieldDef.name || field.name === TypeMetaFieldDef.name || field.name === TypeNameMetaFieldDef.name || field.directives.some(function (_ref) {
    ...
    !type ? process.env.NODE_ENV !== 'production' ? invariant(false, 'SkipClientFieldTransform: Expected type `%s` to be defined in ' + 'the original schema.', rawType.name) : invariant(false) : void 0;
...
function visitFragmentSpread(spread, state) {
  ...
  !(fragment && fragment.kind === 'Fragment') ? process.env.NODE_ENV !== 'production' ? invariant(false, 'SkipClientFieldTransform: Expected a fragment named `%s` to be defined.', spread.name) : invariant(false) : void 0;
