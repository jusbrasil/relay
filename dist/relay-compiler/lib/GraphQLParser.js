...
var partitionArray = require('fbjs/lib/partitionArray');
...
var GraphQLParser = function () {
  ...
  GraphQLParser.prototype._recordVariableReference = function _recordVariableReference(name, type) {
    ...
    if (type && prevType) {
      !(this._referencedVariables[name] == null || isTypeSubTypeOf(this._schema, this._referencedVariables[name], type)) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLParser: Variable `$%s` was used in locations expecting ' + 'the conflicting types `%s` and `%s`. Source: %s.', name, prevType, type, this._getErrorContext()) : invariant(false) : void 0;
    }
  ...
  GraphQLParser.prototype._transformFragment = function _transformFragment(fragment) {
    ...
    var _loop = function _loop(_name) {
      if (_this2._referencedVariables.hasOwnProperty(_name)) {
        ...
        if (localArgument) {
          !(variableType == null || isTypeSubTypeOf(_this2._schema, localArgument.type, variableType)) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLParser: Variable `$%s` was defined as type `%s`, but used ' + 'in a location that expects type `%s`. Source: %s.', _name, localArgument.type, variableType, _this2._getErrorContext()) : invariant(false) : void 0;
        } else {
  ...
  GraphQLParser.prototype._buildArgumentDefinitions = function _buildArgumentDefinitions(fragment) {
    ...
    return args.map(function (arg) {
      ...
      !(argValue.kind === 'Literal') ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLParser: Expected definition for variable `%s` to be an ' + 'object with the following shape: `{type: string, defaultValue?: ' + 'mixed}`, got `%s`. Source: %s.', argValue, _this3._getErrorContext()) : invariant(false) : void 0;
      ...
      !(unknownKeys.length === 0) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLParser: Expected definition for variable `%s` to be an ' + 'object with the following shape: `{type: string, defaultValue?: ' + 'mixed, nonNull?: boolean, list?: boolean}`, got unknown key(s) ' + '`%s`. Source: %s.', argName, unknownKeys.join('`, `'), _this3._getErrorContext()) : invariant(false) : void 0;
  ...
  GraphQLParser.prototype._transformArgumentDefinitions = function _transformArgumentDefinitions(argumentDefinitions) {
    ...
    return argumentDefinitions.map(function (def) {
      ...
      !(defaultLiteral === null || defaultLiteral.kind === 'Literal') ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLParser: Expected null or Literal default value, got: `%s`. ' + 'Source: %s.', defaultLiteral && defaultLiteral.kind, _this4._getErrorContext()) : invariant(false) : void 0;
      return {
        kind: 'LocalArgumentDefinition',
        metadata: null,
        name: name,
  ...
  GraphQLParser.prototype._transformSelections = function _transformSelections(selectionSet, parentType) {
    ...
    return selectionSet.selections.map(function (selection) {
      ...
      if (selection.kind === 'Field') {
        ...
        !false ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLParser: Unexpected AST kind `%s`. Source: %s.', selection.kind, _this5._getErrorContext()) : invariant(false) : void 0;
      }
      ...
      !(conditionalNodes.length === 1) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLParser: Expected exactly one conditional node, got `%s`. ' + 'Source: %s.', conditionalNodes.length, _this5._getErrorContext()) : invariant(false) : void 0;
  ...
  GraphQLParser.prototype._transformFragmentSpread = function _transformFragmentSpread(fragment, parentType) {
    ...
    if (argumentDirectives.length) {
      args = (argumentDirectives[0].arguments || []).map(function (arg) {
        ...
        !(argValue.kind === 'Variable') ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLParser: All @arguments() args must be variables, got %s. ' + 'Source: %s.', argValue.kind, _this6._getErrorContext()) : invariant(false) : void 0;
  ...
  GraphQLParser.prototype._transformHandle = function _transformHandle(fieldName, fieldArgs, clientFieldDirectives) {
    ...
    clientFieldDirectives.forEach(function (clientFieldDirective) {
      ...
      if (handleArgument) {
        ...
        !(maybeHandle.kind === 'Literal' && typeof maybeHandle.value === 'string') ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLParser: Expected the %s argument to @%s to be a literal ' + 'string, got `%s` on field `%s`. Source: %s.', CLIENT_FIELD_HANDLE, CLIENT_FIELD, maybeHandle, fieldName, _this7._getErrorContext()) : invariant(false) : void 0;
        ...
        if (keyArgument) {
          ...
          !(maybeKey.kind === 'Literal' && typeof maybeKey.value === 'string') ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLParser: Expected %s argument to @%s to be a literal ' + 'string, got `%s` on field `%s`. Source: %s.', CLIENT_FIELD_KEY, CLIENT_FIELD, maybeKey, fieldName, _this7._getErrorContext()) : invariant(false) : void 0;
          key = maybeKey.value;
        }
        ...
        if (filtersArgument) {
          ...
          })) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLParser: Expected %s argument to @%s to be an array of ' + 'argument names on field `%s`, but get %s. Source: %s.', CLIENT_FIELD_FILTERS, CLIENT_FIELD, fieldName, maybeFilters, _this7._getErrorContext()) : invariant(false) : void 0;
          // $FlowFixMe
  ...
  GraphQLParser.prototype._transformDirectives = function _transformDirectives(directives) {
    ...
    return directives.map(function (directive) {
      ...
      !directiveDef ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLParser: Unknown directive `@%s`. Source: %s.', name, _this8._getErrorContext()) : invariant(false) : void 0;
  ...
  GraphQLParser.prototype._transformArguments = function _transformArguments(args, argumentDefinitions) {
    ...
    return args.map(function (arg) {
      ...
      !argDef ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLParser: Unknown argument `%s`. Source: %s.', argName, _this9._getErrorContext()) : invariant(false) : void 0;
  ...
  GraphQLParser.prototype._splitConditions = function _splitConditions(mixedDirectives) {
    ...
    mixedDirectives.forEach(function (directive) {
      if (directive.name === INCLUDE || directive.name === SKIP) {
        ...
        !(arg && arg.name === IF) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLParser: Expected an `if` argument to @%s. Source: %s.', directive.name, _this10._getErrorContext()) : invariant(false) : void 0;
        !(arg.value.kind === 'Variable' || arg.value.kind === 'Literal') ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLParser: Expected the `if` argument to @%s to be a variable. ' + 'Source: %s.', directive.name, _this10._getErrorContext()) : invariant(false) : void 0;
        conditions.push({
          kind: 'Condition',
          condition: arg.value,
          metadata: null,
      ...
      case 'ObjectValue':
        ...
        ast.fields.forEach(function (field) {
          ...
          if (type) {
            ...
            !(objectType instanceof GraphQLInputObjectType) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLParser: Expected a value matching type `%s`, but ' + 'got an object value. Source: %s.', type, _this11._getErrorContext()) : invariant(false) : void 0;
            var fieldConfig = objectType.getFields()[fieldName];
            !fieldConfig ? process.env.NODE_ENV !== 'production' ? invariant(false, 'GraphQLParser: Unknown field `%s` on type `%s`. Source: %s.', fieldName, type, _this11._getErrorContext()) : invariant(false) : void 0;
