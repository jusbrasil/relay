...
function createDeferrableReference(context, spread) {
  ...
  !doTypesOverlap(schema, fragmentType, nodeType) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayDeferrableFragmentsTransform: Cannot defer %s since objects of ' + 'type %s can never also be of type Node.', spread.name, fragmentType) : invariant(false) : void 0;
  !(spread.args.length === 0) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayDeferrableFragmentsTransform: Cannot defer %s with arguments.', spread.name) : invariant(false) : void 0;
...
function createDeferrableOperation(context, fragment) {
  ...
  !queryType ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayDeferrableFragmentTransform: "Query" must be a defined type') : invariant(false) : void 0;
  var nodeField = queryType.getFields().node;
  !nodeField ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayDeferrableFragmentTransform: "Query" must define the field "node"') : invariant(false) : void 0;
  var idArg = nodeField.args.find(function (arg) {
    return arg.name === 'id';
  });
  !(idArg && isInputType(idArg.type)) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayDeferrableFragmentTransform: "node" field must define the argument "id"') : invariant(false) : void 0;
...
function getNodeType(schema) {
  ...
  !(nodeType instanceof GraphQLInterfaceType) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayDeferrableFragmentTransform: Schema must define the interface "Node".') : invariant(false) : void 0;
  return nodeType;
}
...
function getIdType(schema) {
  ...
  !idField ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayDeferrableFragmentTransform: "Node" must define the field "id"') : invariant(false) : void 0;
  var idType = getNamedType(idField.type);
  !isInputType(idType) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayDeferrableFragmentTransform: "Node" must define the scalar field "id"') : invariant(false) : void 0;
  return idType;
}
