...
var RelayOSSQuery = function (_RelayQueryNode2) {
  ...
  RelayOSSQuery.create = function create(concreteNode, metaRoute, variables) {
    ...
    !operation ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayQueryRoot.create(): Expected a value created with graphql`query { ... }` ' + '(using the `graphql` tag), got: %s', concreteNode) : invariant(false) : void 0;
    var rootContext = createRootContext(metaRoute, variables);
  ...
  function RelayOSSQuery(concreteNode, rootContext, variables) {
    ...
    !(concreteNode && concreteNode.operation === 'query') ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayQueryRoot.create(): Expected a value created with graphql`query { ... }` ' + '(using the `graphql` tag), got: %s', concreteNode) : invariant(false) : void 0;
...
var RelayQueryOperation = function (_RelayQueryNode3) {
  ...
  function RelayQueryOperation(concreteNode, rootContext, variables) {
    ...
    !(_this5.constructor.name !== 'RelayQueryOperation') ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayQueryOperation: Abstract class cannot be instantiated.') : invariant(false) : void 0;
    return _this5;
  }
  ...
  if (kind === 'Field') {
    ...
    if (hoistedRootArgs) {
      ...
      hoistedRootArgs.forEach(function (argName) {
        ...
        !(rootValue !== undefined || variables.hasOwnProperty(argName)) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayQueryNode: Expected root argument `%s` on unmasked fragment `%s` to be provided ' + 'in the query variables. If the route is still using ` RelayClassic.QL`, please make ' + 'sure that the argument is defined in the injected Relay constants module or add the ' + 'argument in the `prepareParams` for that route.', argName, fragment.name) : invariant(false) : void 0;
