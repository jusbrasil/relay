...
var RelayMutation = function () {
  ...
  RelayMutation.getFragment = function getFragment(fragmentName, variableMapping) {
    ...
    if (!fragmentBuilder) {
      !false ? process.env.NODE_ENV !== 'production' ? invariant(false, '%s.getFragment(): `%s` is not a valid fragment name. Available ' + 'fragments names: %s', this.name, fragmentName, Object.keys(fragments).map(function (name) {
        return '`' + name + '`';
