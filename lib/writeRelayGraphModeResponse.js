...
var CACHE_KEY = RelayGraphModeInterface.CACHE_KEY,
    DEFERRED_FRAGMENTS = RelayGraphModeInterface.DEFERRED_FRAGMENTS,
    FRAGMENTS = RelayGraphModeInterface.FRAGMENTS,
    REF_KEY = RelayGraphModeInterface.REF_KEY,
    PUT_EDGES = RelayGraphModeInterface.PUT_EDGES,
    PUT_NODES = RelayGraphModeInterface.PUT_NODES,
    PUT_ROOT = RelayGraphModeInterface.PUT_ROOT;
...
var RelayGraphModeWriter = function () {
  ...
  RelayGraphModeWriter.prototype.write = function write(payload) {
    ...
    payload.forEach(function (operation) {
      if (operation.op === PUT_ROOT) {
        ...
        !false ? process.env.NODE_ENV !== 'production' ? invariant(false, 'writeRelayGraphModeResponse(): Invalid operation type `%s`, ' + 'expected `root`, `nodes`, or `edges`.', operation.op) : invariant(false) : void 0;
      }
    });
  };
  ...
  RelayGraphModeWriter.prototype._writeEdges = function _writeEdges(operation) {
    ...
    var range = operation.range,
        ...
        pageInfo = operation.pageInfo;

    var rangeID = this._cacheKeyMap.get(range[CACHE_KEY]);
    !rangeID ? process.env.NODE_ENV !== 'production' ? invariant(false, 'writeRelayGraphModeResponse(): Cannot find a record for cache key ' + '`%s`.', range[CACHE_KEY]) : invariant(false) : void 0;
    !RelayConnectionInterface.hasRangeCalls(args) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'writeRelayGraphModeResponse(): Cannot write edges for connection on ' + 'record `%s` without `first`, `last`, or `find` argument.', rangeID) : invariant(false) : void 0;
    ...
    edges.forEach(function (edgeData) {
      ...
      !(typeof nodeData === 'object') ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayQueryWriter: Expected node to be an object for `%s`.', edgeData) : invariant(false) : void 0;
  ...
  RelayGraphModeWriter.prototype._writePlural = function _writePlural(dataID, storageKey, nextValue) {
    ...
    nextValue.forEach(function (nextItem) {
      if (nextItem == null) {
        ...
        !!nextScalars ? process.env.NODE_ENV !== 'production' ? invariant(false, 'writeRelayGraphModeResponse(): Expected items for field `%s` to ' + 'all be objects or all be scalars, got both.', storageKey) : invariant(false) : void 0;
        ...
        // array of scalars
        !!nextIDs ? process.env.NODE_ENV !== 'production' ? invariant(false, 'writeRelayGraphModeResponse(): Expected items for field `%s` to ' + 'all be objects or all be scalars, got both.', storageKey) : invariant(false) : void 0;
