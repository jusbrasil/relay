...
var isClassicRelayContext = require('./isClassicRelayContext');
...
function createContainerComponent(Component, spec) {
  ...
  var RelayContainer = function (_React$Component) {
    ...
    function RelayContainer(props, context) {
      ...
      !isClassicRelayContext(relay) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayContainer: `%s` was rendered with invalid Relay context `%s`. ' + 'Make sure the `relay` property on the React context conforms to the ' + '`RelayEnvironment` interface.', containerName, relay) : invariant(false) : void 0;
    ...
    RelayContainer.prototype._getQueryData = function _getQueryData(props) {
      ...
      forEachObject(this._fragmentResolvers, function (fragmentResolver, propName) {
        ...
        if (!propValue || !fragmentPointer) {
          ...
          !fragmentResolver ? process.env.NODE_ENV !== 'production' ? invariant(false, 'fragmentResolver should not be null') : invariant(false) : void 0;
