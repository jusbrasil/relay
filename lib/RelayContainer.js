      ...
      fragmentNames.forEach(function (fragmentName) {
        ...
        if (fragment.isPlural()) {
          var _ret3 = function () {
            ...
            if (dataIDs) {
              !(dataIDs.length === propValue.length) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayContainer: Invalid prop `%s` supplied to `%s`. Some ' + 'array items contain data fetched by Relay and some items ' + 'contain null/mock data.', fragmentName, componentName) : invariant(false) : void 0;
            }
            dataIDOrIDs = dataIDs;
          }();
...
function validateFragmentProp(componentName, fragmentName, fragment, prop, prevVariables) {
  var hasFragmentData = RelayFragmentPointer.hasFragment(prop, fragment) || !!prevVariables && RelayContainerComparators.areQueryVariablesEqual(prevVariables, fragment.getVariables());
  if (!hasFragmentData) {
