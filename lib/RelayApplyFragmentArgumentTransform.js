...
function transformCondition(context, fragments, scope, node) {
  ...
  !(condition.kind === 'Literal' || condition.kind === 'Variable') ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayApplyFragmentArgumentTransform: A non-scalar value was applied to ' + 'an @include or @skip directive, the `if` argument value must be a ' + 'variable or a Boolean, got `%s`.', condition) : invariant(false) : void 0;
...
function transformValue(scope, value) {
  if (value.kind === 'Variable') {
    ...
    !(scopeValue != null) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayApplyFragmentArgumentTransform: variable `%s` is not in scope.', value.variableName) : invariant(false) : void 0;
    return scopeValue;
...
function transformFragment(context, fragments, parentScope, fragment, args) {
  ...
  !(!fragments.has(fragmentName) || fragments.get(fragmentName) !== undefined) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayApplyFragmentArgumentTransform: Found a circular reference from ' + 'fragment `%s`.', fragment.name) : invariant(false) : void 0;
...
function hashArguments(args, scope) {
  ...
  var printedArgs = JSON.stringify(sortedArgs.map(function (arg) {
    ...
    if (arg.value.kind === 'Variable') {
      ...
      !(value != null) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayApplyFragmentArgumentTransform: variable `%s` is not in scope.', arg.value.variableName) : invariant(false) : void 0;
    } else {
      value = arg.value;
    }
    return {
