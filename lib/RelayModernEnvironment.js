...
var deferrableFragmentKey = require('./deferrableFragmentKey');
    ...
    return this._network.execute(operation.node, operation.variables, cacheConfig || {})['do']({
      next: function next(executePayload) {
        ...
        if (executePayload.isOptimistic) {
          !(optimisticResponse == null) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'environment.execute: only support one optimistic respnose per ' + 'execute.') : invariant(false) : void 0;
          optimisticResponse = {
            source: source,
