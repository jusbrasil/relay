...
function validateConnectionType(definitionName, type) {
  ...
  !(nodeType instanceof GraphQLInterfaceType || nodeType instanceof GraphQLUnionType || nodeType instanceof GraphQLObjectType) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayConnectionTransform: Expected type `%s` to have an %s.%s field' + 'for which the type is an interface, object, or union in document `%s`.', type, EDGES, NODE, definitionName) : invariant(false) : void 0;

  var cursor = edgeType.getFields()[CURSOR];
  !(cursor && SchemaUtils.getNullableType(cursor.type) instanceof GraphQLScalarType) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayConnectionTransform: Expected type `%s` to have an ' + '%s.%s field for which the type is a scalar in document `%s`.', type, EDGES, CURSOR, definitionName) : invariant(false) : void 0;
  ...
  !(pageInfoType instanceof GraphQLObjectType) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayConnectionTransform: Expected type `%s` to have a %s field for ' + 'which the type is an object in document `%s`.', type, PAGE_INFO, definitionName) : invariant(false) : void 0;
  ...
  [END_CURSOR, HAS_NEXT_PAGE, HAS_PREV_PAGE, START_CURSOR].forEach(function (fieldName) {
    ...
    !(pageInfoField && SchemaUtils.getNullableType(pageInfoField.type) instanceof GraphQLScalarType) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayConnectionTransform: Expected type `%s` to have an ' + '%s field for which the type is an scalar in document `%s`.', pageInfo.type, fieldName, definitionName) : invariant(false) : void 0;
  });
}
