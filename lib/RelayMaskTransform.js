...
function visitFragment(fragment, state) {
  ...
  state.hoistedArgDefs.forEach(function (hoistedArgDef, argName) {
    ...
    if (existingArgDef) {
      !areSameArgumentDefinitions(existingArgDef, hoistedArgDef.argDef) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayMaskTransform: Cannot unmask fragment spread `%s` because ' + 'argument `%s` has been declared in `%s` and they are not the same.', hoistedArgDef.source, argName, fragment.name) : invariant(false) : void 0;
      return;
    }
...
function visitFragmentSpread(fragmentSpread, state) {
  ...
  }) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayMaskTransform: Cannot unmask fragment spread `%s` because it has local ' + 'argument definition.', fragmentSpread.name) : invariant(false) : void 0;
  ...
  try {
    for (var _iterator = fragment.argumentDefinitions[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      if (hoistedArgDef) {
        !areSameArgumentDefinitions(_argDef, hoistedArgDef.argDef) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayMaskTransform: Cannot unmask fragment spread `%s` because ' + 'argument `%s` has been declared in `%s` and they are not the same.', hoistedArgDef.source, _argDef.name, fragmentSpread.name) : invariant(false) : void 0;
        continue;
      }
