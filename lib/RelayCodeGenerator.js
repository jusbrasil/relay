...
function generate(node) {
  !(['Batch', 'Fragment'].indexOf(node.kind) >= 0) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayCodeGenerator: Unknown AST kind `%s`. Source: %s.', node.kind, getErrorMessage(node)) : invariant(false) : void 0;
...
var RelayCodeGenVisitor = {
  leave: {
    Batch: function Batch(node) {
      !(node.requests.length !== 0) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'Batch must contain Requests.') : invariant(false) : void 0;
    ...
    Condition: function Condition(node, key, parent, ancestors) {
      !(node.condition.kind === 'Variable') ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayCodeGenerator: Expected static `Condition` node to be ' + 'pruned or inlined. Source: %s.', getErrorMessage(ancestors[0])) : invariant(false) : void 0;
      return {
        kind: 'Condition',
    ...
    Argument: function Argument(node, key, parent, ancestors) {
      !(['Variable', 'Literal'].indexOf(node.value.kind) >= 0) ? process.env.NODE_ENV !== 'production' ? invariant(false, 'RelayCodeGenerator: Complex argument values (Lists or ' + 'InputObjects with nested variables) are not supported, argument ' + '`%s` had value `%s`. Source: %s.', node.name, JSON.stringify(node.value, null, 2), getErrorMessage(ancestors[0])) : invariant(false) : void 0;
